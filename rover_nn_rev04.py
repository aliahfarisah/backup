...
def execute_movement(id_rover):
    try:
        ...
        csv_columns = ['timestamp',
                       ...
                       'angle', 'flipped_angle', 'distance_moved', 'time_difference']
        ...
        while not device_manager.stop_event.is_set():
            ...
            if prev_rover_pos is not None and prev_time is not None:
                ...
                distance_moved = np.sqrt((kalman_avg_x - prev_rover_pos[0])**2 + (kalman_avg_y - prev_rover_pos[1])**2)
                print("Distance moved:", distance_moved)
                
                if delta_t > 0:
                    transform_vel_x = distance_moved / delta_t
                else:
                    transform_vel_x = 0.0
                transform_vel_y = 0.0  # Assuming no vertical velocity
            ...
            if distance_to_target < 10:  # Stop within 10 
                print(f"Rover reached the target. Stopping rover.")
