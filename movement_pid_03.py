...
def execute_movement(id_rover, file_path):
        ...
        if index > 0:
            ...
            if xr is not None and yr is not None:
                try:
                    ...
                    df.at[index, 'T_rover'] = datetime.datetime.now().strftime('%Y%m%d%H%M%S.%f')
                ...
                actual_angle = df.at[index, 'Angle_r (deg)']
                desired_angle = df.at[index, 'Angle_s (deg)']
                
                dt = time.time() - start_time
                start_time = time.time()
                

                if pd.notna(actual_angle) and pd.notna(desired_angle):
                    angle_correction = pid.update(actual_angle, dt)
                    adjusted_angle = actual_angle + angle_correction

                    # Store PID values in the DataFrame
                    df.at[index, 'angle_correction'] = angle_correction
                    df.at[index, 'adjusted_angle'] = adjusted_angle
    ...
    df['T_rover'] = pd.to_datetime(df['T_rover'], format='%Y%m%d%H%M%S.%f')
