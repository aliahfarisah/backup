...
def main():
    uwb_server = Uwb()
    daemon = Pyro5.api.Daemon(host="192.168.50.141", port=9091)
    uri = daemon.register(uwb_server, "uwb")
    
    thread = threading.Thread(target=rover_server.start_connection, args=(5,))
    thread.daemon = True
    thread.start()
