...
import time
import datetime
from rover06 import Rover  # Import the Rover class from rover06.py
...
class MainApp:
    ...
    def start_execution(self):
        tclick = datetime.datetime.now().time()
        print(f"tclick = {tclick}")
        current_hour = tclick.hour
        current_minute = tclick.minute
        current_second = tclick.second
        ...
        print(f"texecute: {adjusted_hour}:{adjusted_minute}:{adjusted_second}")
        self.execute_btn.config(state=tk.NORMAL)
