        ...
        if self.is_connected:
            if self.check_csv_exists():
                try:
                    ...
                    command = 'cd Desktop/lightShow && sudo python3 00_import_csv_rev04.py'
                    stdin, stdout, stderr = ssh_client.exec_command(command)
                ...
                print(f"CSV file for Rover {self.rover_id} is missing. Cannot execute movement.")
        else:
            print(f"Rover {self.rover_id} is not connected. Cannot execute movement.")
