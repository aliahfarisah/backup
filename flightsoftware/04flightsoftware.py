...
class MainApp:
    ...
    def connect_rover(self, ip_address, index):
        rov = Rover(ip_address, index, self.update_status)
        self.rov[index] = rov
        connection_thread = threading.Thread(target=rov.connect)
        connection_thread.start()

    def update_status(self, index, color):
        if color == 'green':
            self.canvas.itemconfig(self.circles[index], fill='green')
        elif color == 'red':
            self.canvas.itemconfig(self.circles[index], fill='red')
        else:
            self.canvas.itemconfig(self.circles[index], fill='#4D4E6D')

    def stop_stream(self):
        if self.is_streaming:
            self.is_streaming = False
            
            for rover_connection in self.rov:
                if rover_connection:
                    rover_connection.disconnect()
