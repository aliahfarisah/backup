...
class Rover:
    ...
    def verify_identity(self):
        try:
            ...
            with Pyro5.api.Proxy(f"PYRO:server@{self.ip_address}:9090") as identity_service:
    ...
    def send_go_signal(self):
        if self.id_rover_verified:
            try:
                # Connect to the remote Pyro server to trigger movement
                with Pyro5.api.Proxy(f"PYRO:move@{self.ip_address}:9093") as movement_service:
                    movement_service.trigger_start()  # This sends the 'go' signal to the server
                    print(f"'Go' signal sent to Rover {self.rover_id}.")
            except Pyro5.errors.CommunicationError as e:
                print(f"Communication error sending 'go' signal for Rover {self.rover_id}: {e}")
            except Exception as e:
                print(f"Error sending 'go' signal for Rover {self.rover_id}: {e}")
        else:
            print(f"Cannot send 'go' signal. Rover {self.rover_id} identity not verified.")
