...
class MainApp:
    def __init__(self, root, window_title):
        ...
        self.file_service = Pyro5.api.Proxy("PYRONAME:example.fileservice")
        ...
        self.circle_positions = [(140, 170), (318, 170), (496, 170), (674, 170), (852, 170)]
        self.circles = []
        self.circle_texts = []
        ...
        for i, pos in enumerate(self.circle_positions):
            ...
            self.canvas.create_text(x - 40, y + 70, text="Animation :", fill='black', font=('Times', 12))
            ...
            self.canvas.create_window(x, y + 200, window=upload_btn)
    ...
    def transfer_data(self):
        ...
        if file_path:
            try:
                ...
                result = self.file_service.receive_csv_data(filename, csv_data)
                self.canvas.itemconfig(self.status_text, text=result)
            
            except FileNotFoundError:
                self.canvas.itemconfig(self.status_text, text="File not found.")
            except Exception as e:
                self.canvas.itemconfig(self.status_text, text=f"Error: {str(e)}")
