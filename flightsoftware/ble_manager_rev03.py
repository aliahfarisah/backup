...
class BLEDeviceManager:
    def __init__(self):
        ...
        self.device_info = pd.DataFrame(columns=['Name', 'Status', 'X', 'Y', 'Z', 'T_tag'])
        self.all_device_info = pd.DataFrame(columns=['T_tag', 'X', 'Y', 'Z'])
    ...
    async def read_characteristic(self, device_address, device_name, characteristic_uuid, locData_uuid):
        while not self.stop_event.is_set():  # Check if stop event is set
            ...
            try:
                ...
                while client.is_connected and not self.stop_event.is_set():  # Check if stop event is set
                    try:
                        ...
                        x_dec = int(x_coord, 16)/1000
                        y_dec = int(y_coord, 16)/1000
                        z_dec = int(z_coord, 16)/1000
                        ...
                        with self.lock:
                            ...
                            self.device_info.loc[self.device_info['Name'] == device_name, 'T_tag'] = current_time
                        ...
                        csv_file_path = f'device_info_{timestamp}.csv'
                        self.save_to_csv(csv_file_path)

                        # Calculate time difference if previous_time is set
                        if self.previous_time is not None:
                            prev_time_obj = datetime.datetime.strptime(self.previous_time, '%Y%m%d %H%M%S.%f')
                            current_time_obj = datetime.datetime.strptime(current_time, '%Y%m%d %H%M%S.%f')
                            time_diff = (current_time_obj - prev_time_obj).total_seconds() * 1000
                            print(f"Time difference from previous update: {time_diff:.2f} ms")
                            
                            # Add time difference to the all_device_info DataFrame
                            self.all_device_info.loc[self.all_device_info.index[-1], 'Time_Diff [msec]'] = time_diff
    ...
    def start_connection(self, id_rover):
        async def inner():
            ...
            for device in self.filtered_devices:
                ...
                #data_task = asyncio.create_task(self.read_characteristic(device_address, device_name, characteristic_uuid, locData_uuid))
    ...
    def save_to_csv(self, file_path):
        with self.lock:
            try:
                self.all_device_info.to_csv(
                    file_path,
                    header=True,
                    index=False,
                    line_terminator='\n'
                )
                print(f"Saved {len(self.all_device_info)} entries to {file_path}")
