    ...
    def update_time_from_server(self, circle_index):
        try:
            ...
            if ip_address:
                ...
                thread = threading.Thread(target=self.fetch_time, args=(circle_index, ip_address))
                thread.daemon = True
                thread.start()
            else:
                print(f"No IP address found for Rover {circle_index}.")
        except Exception as e:
            print(f"Error connecting to time service for Rover {circle_index}: {str(e)}")

    def fetch_time(self, circle_index, ip_address):
        try:
            # Create a new proxy inside the thread
            time_service = Pyro5.api.Proxy(f"PYRO:time@{ip_address}:9092")
            ...
            print(f"Communication error with time service for Rover {circle_index}: {str(ce)}")
        except Exception as e:
            print(f"Error fetching time for Rover {circle_index}: {str(e)}")
    ...
    def refresh_file_list_for_rover(self, circle_index):
        try:
            ...
            if ip_address:
                ...
                file_service = Pyro5.api.Proxy(f"PYRO:file@{ip_address}:9091")
    ...
    def transfer_data(self, circle_index):
        try:
            ...
            if ip_address:
                ...
                self.message_label.place(x=50, y=650)  # Adjust x coordinate as needed
            else:
                print(f"No IP address found for Rover {rover_id}.")
