...
class MainApp:
    def __init__(self, root, window_title):
        ...
        for i, pos in enumerate(self.circle_positions):
            ...
            # Add labels for displaying rover positions below the canvas
            label = ttk.Label(root, text=f"Rover {i + 1}: Pos(---, ---, ---)", font=("Times", 12))
            label.place(x=x - 60, y=600)  # Placing the label below the canvas
            self.rover_labels.append(label)
        ...
        for i, pos in enumerate(self.circle_positions):
            ...
            refresh_button.place(x=x + 60, y=y + 250)  
            
            go_button = ttk.Button(root, text="Go", command=lambda index=i: self.send_go_signal_for_rover(index))
            go_button.place(x=x + 20, y=y + 530)  # Adjust position as needed
        ...
        self.start_btn = ttk.Button(root, text="Start", style='W.TButton', command=self.stop_stream)
        self.start_btn.place(x=1200, y=580, width=150, height=50)
        ...
        self.rover_position_label = ttk.Label(root, text="Rover 1 Position: ---")
        self.rover_position_label.place(x=1200, y=500)
    ...
    def get_ip_and_port_for_rover(self, circle_index):
        try:
            # Read the IP addresses from ips.csv
            ips_df = pd.read_csv('ip.csv')
            
            # Read the ports from ports.csv
            ports_df = pd.read_csv('rovers.csv')
            
            if circle_index < len(ips_df) and circle_index < len(ports_df):
                # Get the IP address and port for the specific rover (matching by ID or by circle_index)
                rover_id = ips_df.loc[circle_index, 'ID']  # Assuming the ID column is in both CSVs
                
                # Get the IP address
                ip_address = ips_df.loc[circle_index, 'IP']
                
                # Get the port by matching the same rover ID in ports.csv
                port = ports_df[ports_df['ID'] == rover_id]['Port'].values[0]
            ...
            print(f"Error fetching IP and port for rover {circle_index}: {e}")
            return None, None
        ...
        if not self.is_streaming:
            ...
            if ip_choice == "Use Fixed IP":
                try:
                    ...
                    for i in range(len(self.circle_positions)):  # Assume each rover corresponds to a circle
                        thread = threading.Thread(target=self.fetch_uwb_data_from_rover, args=(i,))
                        thread.daemon = True  # Daemonize thread to exit when the program exits
                        thread.start()
    ...
    def connect_rover(self, rover_id, ip_address, index):
        ...
        self.rovers[str(rover_id)] = rover
    ...
    # Function to send the Go signal
    def send_go_signal_for_rover(self, circle_index):
       
        try:
            rover_id = circle_index + 1  # Assuming rover IDs are 1-indexed
            rover = self.rovers.get(str(rover_id))
            
            if rover:
                rover.send_go_signal()
                self.message_label.config(text=f"'Go' signal sent to Rover {rover_id}.")
            else:
                self.message_label.config(text=f"Rover {rover_id} not connected.")
                
        except Exception as e:
            self.message_label.config(text=f"Error sending 'Go' signal to Rover {circle_index + 1}: {str(e)}")
            
    def fetch_uwb_data_from_rover(self, circle_index):
        ip_address, port = self.get_ip_and_port_for_rover(circle_index)
        
        try:
            # Use both IP address and port in the Pyro5 Proxy connection
            with Pyro5.api.Proxy(f"PYRO:coordinates@{ip_address}:{port}") as proxy:
                while self.is_streaming:
                    # Fetch the UWB data from the server (rover)
                    uwb_data = proxy.get_uwb_data()  # Fetch the UWB data generated by the rover

                    if uwb_data:
                        print(f"Rover {circle_index + 1} UWB Data received: {uwb_data}")

                        try:
                            # Step 1: Extract num_anchors and fetched_rover_count from the data
                            num_anchors = int(uwb_data[1])
                            fetched_rover_count = int(uwb_data[2])

                            print(f"Rover {circle_index + 1}: Number of anchors: {num_anchors}, Fetched Rover Count: {fetched_rover_count}")

                            # Step 2: Process data for each rover
                            index = 3
                            for _ in range(fetched_rover_count):  # Process each rover's data
                                rover_id = uwb_data[index]  # Rover ID
                                x = float(uwb_data[index + 1])
                                y = float(uwb_data[index + 2])
                                z = float(uwb_data[index + 3])

                                # Find the corresponding circle index using rover_id
                                if rover_id in self.rover_ids:
                                    rover_circle_index = self.rover_ids[rover_id]

                                    # Log which rover's data was received by which rover
                                    print(f"Rover {circle_index + 1} received data from Rover {rover_id}: Pos({x}, {y}, {z})")

                                    # Update the UI to display the rover and source
                                    self.window.after(0, self.update_rover_position, rover_circle_index, rover_id, x, y, z, circle_index + 1)

                                # Move to the next rover's data block
                                index += 4

                        except Exception as e:
                            print(f"Error displaying UWB data for rover {circle_index + 1}: {str(e)}")

                    time.sleep(1)

        except Exception as e:
            print(f"Error fetching UWB data for rover {circle_index + 1}: {e}")

    # Define a method to update the UI for the rover's position and the source
    def update_rover_position(self, circle_index, rover_id, x, y, z, source_rover_id):
        
        # Update the label below the canvas with rover position and the source
        self.rover_labels[circle_index].config(text=f"From Rover {source_rover_id}: {rover_id}: Pos({x:.2f}, {y:.2f}, {z:.2f})")
