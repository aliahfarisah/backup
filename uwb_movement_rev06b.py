...
def execute_movement(theta_xy, R, G, B):                   
    #Move rover
    chassis.set_velocity(50, theta_xy, 0)
...
def start_movement(id_rover):
    ...
    pid = PID(P=0.2, I=0.0, D=0.0)  # Adjust these values for fine-tuning
    pid.setWindup(10)  # Set the windup guard to avoid integral windup
...
#                 print(f"Starting movement for Rover {id_rover}.")
                ...
                for index, row in df.iterrows():      
                    ...
                    if index != (num_rows - 1):
                        ...
                        pid.SetPoint = theta_xy
                        
                        # Update the PID controller with current feedback (actual theta)
                        pid.update(theta_xy)
                        
                        # Get the corrected theta from the PID output
                        corrected_theta = pid.output
                        
                        # Print raw and corrected angles for diagnosis
                        print(f"Raw theta_xy: {theta_xy}")
                        print(f"Corrected theta_xy: {corrected_theta}")
                        df.at[index, 'Corrected theta_xy'] = corrected_theta

                        execute_movement(corrected_theta, row['Red'], row['Green'], row['Blue'])
