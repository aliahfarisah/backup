...
def execute_movement(id_rover, file_path):
    ...
    for index, row in df.iterrows():
        if index > 0:
            ...
            if pd.notna(current_angle) and pd.notna(pid_angle.setpoint):
                ...
                adjusted_angle = control_signal  # Calculate the adjusted angle
    ...
    df.to_csv('updated_movement3.csv', index=False)
    print(f"Saved updated DataFrame to 'updated_movement3.csv'")
