                ...
                try:
                    ...
                    if idx_pos != -1:
                        ...
                        with data_lock:
                            ...
                            #print(f"Updated xr: {xr}")
                        
                        #return pos_x, pos_y, x_smooth, y_smooth, xr, yr, timestamp

                except (ValueError, IndexError) as e:
                    print(f"Error parsing serial data: {e}, Response: {response}")
...
def execute_movement(id_rover, file_path, ser):
    ...
    coord_thread = threading.Thread(target=update_coordinates_from_serial, args=(ser,))
    coord_thread.start()
    ...
    try:
        for index, row in df.iterrows():
            ...
            with data_lock:
                if 'latest_data' in globals() and latest_data:
                    raw_x = latest_data.get('pos_x')
                    raw_y = latest_data.get('pos_y')
                    x_smooth = latest_data.get('x_smooth')
                    y_smooth = latest_data.get('y_smooth')
...
#                     print("xr:", xr)
                    ...
                    if raw_x is not None and raw_y is not None and xr is not None and yr is not None:
                        ...
                        if pd.notna(df.at[index, 'Angle_s (deg)']) and pd.notna(df.at[index, 'Angle_c (deg)']):
                            chassis.set_velocity(50, df.at[index, 'Angle_s (deg)'], 0)
                        else:
