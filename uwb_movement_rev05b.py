...
def execute_movement(theta_xy, R, G, B):                   
    #Move rover
    chassis.set_velocity(100, theta_xy, 0)
    ...
    try:
        ...
        while True:
            if uwb_is_connected:
                ...
                # Wait for a signal from the client (instead of manual input)
                print(f"Rover {id_rover} is ready and waiting for the client to send the 'go' signal.")
                movement_event.wait()
                ...
                for index, row in df.iterrows():      
                    ...
                    if index != (num_rows - 1):
                        ...
                        execute_movement(theta_xy, row['Red'], row['Green'], row['Blue'])
